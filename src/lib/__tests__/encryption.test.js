...
describe('EncryptionService', () => {
  ...
  describe('encrypt', () => {
    ...
    it('should throw error for missing data', async () => {
      await expect(EncryptionService.encrypt(null, userKey))
        .rejects.toThrow('Data and user key are required for encryption');
    });

    it('should throw error for missing key', async () => {
      await expect(EncryptionService.encrypt(testData, null))
        .rejects.toThrow('Data and user key are required for encryption');
    });
  ...
  describe('decrypt', () => {
    ...
    it('should throw error for wrong key', async () => {
      const encrypted = await EncryptionService.encrypt(testData, userKey);
      const wrongKey = EncryptionService.generateUserKey('wrong-user', 'wrong-password');
    ...
    it('should throw error for missing encrypted data', async () => {
      await expect(EncryptionService.decrypt(null, userKey))
        .rejects.toThrow('Encrypted data and user key are required for decryption');
    });

    it('should throw error for missing key', async () => {
      const encrypted = await EncryptionService.encrypt(testData, userKey);
      
      await expect(EncryptionService.decrypt(encrypted, null))
        .rejects.toThrow('Encrypted data and user key are required for decryption');
    });

    it('should throw error for invalid encrypted data format', async () => {
      const invalidData = { ciphertext: 'invalid', algorithm: 'AES-256-CBC-HMAC' };
      
      await expect(EncryptionService.decrypt(invalidData, userKey))
    ...
    it('should throw error for unsupported algorithm', async () => {
      ...
      await expect(EncryptionService.decrypt(encrypted, userKey))
  ...
  describe('key management', () => {
    ...
    it('should clear specific user key', () => {
      ...
      const newKey = EncryptionService.generateUserKey(testUserId, testPassword);
  ...
  describe('end-to-end encryption/decryption', () => {
    it('should handle multiple encrypt/decrypt cycles', async () => {
      const userKey = EncryptionService.generateUserKey(testUserId, testPassword);
      let data = testData;
